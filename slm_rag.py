...
def prepare_promt(query, n_chunks, application_id=None):
...
def run_gemma_one_query(query, model, tokenizer, n_chunks, application_id=None):
    ...
    promt = prepare_promt(query, n_chunks, application_id)
...
def run_phi_one_query(query, model, tokenizer, n_chunks, application_id=None):
    
    if(N_CHUNKS != -1):
        n_chunks = N_CHUNKS
    
    promt = prepare_promt(query, n_chunks, application_id=None)
